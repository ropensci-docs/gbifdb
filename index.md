...
specify the option `to_duckdb = TRUE` in
[`gbif_remote()`](https://docs.ropensci.org/gbifdb/reference/gbif_remote.md)
(or simply pass the connection to
[`arrow::to_duckdb()`](https://arrow.apache.org/docs/r/reference/to_duckdb.html))
to create a `duckdb` connection. This is slightly slower at this time.
Keep in mind that as with any database connection, to use non-`dplyr`
functions the user will generally need to call
[`dplyr::collect()`](https://dplyr.tidyverse.org/reference/compute.html),
which pulls the data into working memory.  
Be sure to subset the data appropriately first (e.g. with `filter`,
`summarise`, etc), as attempting to
[`collect()`](https://dplyr.tidyverse.org/reference/compute.html) a
large table will probably exceed available RAM and crash your R session!

When using a
[`gbif_remote()`](https://docs.ropensci.org/gbifdb/reference/gbif_remote.md)
connection, all I/O operations will be conducted over the network
storage instead of your local disk, without downloading the full dataset
first. Consequently, this mechanism will perform best on platforms with
faster network connections. These operations will be considerably slower
than they would be if you download the entire dataset first (see below,
unless you are on an AWS cloud instance in the same region as the remote
host), but this does avoid the download step all-together, which may be
necessary if you do not have 100+ GB free storage space or the time to
download the whole dataset first (e.g. for one-off queries).
...
By default, this will download to the dir given by
...
[`select()`](https://dplyr.tidyverse.org/reference/select.html) call at
